...
class ArticleTableController: UITableViewController {
    ...
    func downloadArticleData(){
        let url = NSURL(string: "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20feed%20where%20url=%27www.abc.net.au%2Fnews%2Ffeed%2F51120%2Frss.xml%27&format=json")
        
        print(url)
        
        let downloadProcess = NSURLSession.sharedSession().dataTaskWithURL(url!){
        ...
        downloadProcess.resume()
    }
    
    func parseJsonData(extractedJson: NSData){
        do{
            let jsonResult = try NSJSONSerialization.JSONObjectWithData(
                extractedJson, options: NSJSONReadingOptions.MutableContainers) as! NSDictionary
            
            //var totalArticles = jsonResult["query"]!["count"]!!
            ...
            //for articleArray in ( jsonResult as! NSArray as! [NSDictionary]){
                
             //   var jsonItem = jsonResult["query"]!["results"]!["item"]
